...
// Variáveis do Modal de Customização (necessário para o JS do cardapio funcionar)
// Assumindo que essas variáveis são definidas em outro lugar (ex: modal_carrinho.js)
// Para que o cardapio.js funcione, vamos definir como null aqui se não forem globais
let customizacaoModal = null; 
let listaAdicionaisContainer = null;
...
function rebindAdminElements() {
    ...
    // Inicializa variáveis do modal de customização, se existirem na DOM
    customizacaoModal = document.getElementById('customizacao-modal');
    listaAdicionaisContainer = document.getElementById('lista-adicionais-container');
...
// Função unificada de carregamento (AGORA PRIORIZA O JSON)
async function carregarCardapio() {
    let dadosDoJson = null;
    let dadosDoLocalStorage = carregarCardapioDoLocalStorage(); // Tenta carregar o backup

    // 1. Tenta carregar do JSON (PRIORIDADE)
    try {
        console.log("Tentando carregar cardápio do JSON.");
        const response = await fetch('cardapio.json');
        if (!response.ok) throw new Error('Falha ao carregar cardapio.json');
        dadosDoJson = await response.json();
    } catch (error) {
        console.error("Erro CRÍTICO ao carregar cardápio do JSON:", error);
    }
    
    // 2. Decide qual dado usar
    if (dadosDoJson) {
        // Se o JSON carregou com sucesso, usa ele e ATUALIZA o Local Storage
        console.log("Cardápio carregado e atualizado com o JSON.");
        cardapioData = dadosDoJson;
        salvarCardapioNoLocalStorage(cardapioData); // Salva a versão NOVA no Local Storage
    } else if (dadosDoLocalStorage) {
        // Se o JSON FALHOU, usa o Local Storage como backup de segurança
        console.log("JSON falhou. Cardápio carregado do Local Storage (Backup).");
        cardapioData = dadosDoLocalStorage;
    } else {
        // Se tudo falhou
        console.log("Nenhuma fonte de cardápio encontrada.");
        cardapioData = [];
    }
...
async function carregarCardapioAdmin() {
    // Carrega o JSON primeiro no Admin também para garantir a versão mais recente
...
function handleAdicionarAoCarrinho(event) {
    ...
    if (categoriaId === ID_CATEGORIA_CUSTOMIZAVEL) {
        ...
        // Garante que as funções e variáveis globais existam antes de chamar
        if (typeof mostrarModal === 'function' && customizacaoModal) { 
...
function adicionarItemSimplesAoCarrinho(item) {
    // Busca se existe um item IGUAL e SEM adicionais (simples)
    const existingItem = carrinho.find(c => c.id === item.id && !c.adicionais);

    if (existingItem) {
        existingItem.quantidade++;
    } else {
        // Adiciona a propriedade 'precoTotal' para consistência, se não for customizado
        carrinho.push({ ...item, quantidade: 1, precoTotal: item.preco }); 
    }
    if (typeof updateContadorCarrinho === 'function') updateContadorCarrinho(); 
}

function setupCustomizacaoModal(item) {
    // Rebind das variáveis de customização para evitar erros se o modal foi carregado depois
    customizacaoModal = document.getElementById('customizacao-modal');
    listaAdicionaisContainer = document.getElementById('lista-adicionais-container');
    ...
    adicionaisGlobais.forEach(adicional => {
        ...
        if (typeof formatarMoeda === 'function') {
            // Certifique-se que os atributos estão corretos
            div.innerHTML = `
...
function recalcularAdicionais() {
    ...
    if (listaAdicionaisContainer) {
        listaAdicionaisContainer.querySelectorAll('input[type="checkbox"]:checked').forEach(checkbox => {
            // Usa parseFloat diretamente do atributo
    ...
    if (typeof formatarMoeda === 'function') {
        ...
        if (precoAdicionaisSpan) {
            precoAdicionaisSpan.textContent = formatarMoeda(totalAdicionais);
        }
...
function adicionarItemCustomizadoAoCarrinho() {
    ...
    carrinho.push({
        ...
        precoTotal: precoFinal, // Preço já com os adicionais
        quantidade: 1 
    });
...
// INICIALIZAÇÃO - Garante que o carregamento ocorra ao final
// Não faz mal chamar rebindAdminElements na página do cardapio.html, 
// mas carregarCardapio é essencial.
window.addEventListener('load', () => {
    rebindAdminElements(); 
    verificarAcessoAdmin();
    // Se não for a página de admin, apenas carrega o cardápio
    if (!window.location.pathname.endsWith('admin_jottav_burguer.html')) {
        carregarCardapio(); 
    }

    // Se o botão do modal de customização existir, anexa o listener
    const btnAddCustomizado = document.getElementById('adicionar-customizado-carrinho');
    if (btnAddCustomizado) {
        btnAddCustomizado.addEventListener('click', adicionarItemCustomizadoAoCarrinho);
    }
});